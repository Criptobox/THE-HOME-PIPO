...
import { PizzaSize, CrustType, SauceType, ToppingCoverage, CartItem } from '../types';
import { PIZZA_SIZES, CRUST_OPTIONS, SAUCE_OPTIONS, TOPPINGS } from '../data/menu';
import { Flame, Check, Sparkles, RotateCcw, Plus, Layers } from 'lucide-react';

interface VisualPizzaBuilderProps {
  onAddToCart: (cartItem: CartItem) => void;
}

export const VisualPizzaBuilder: React.FC<VisualPizzaBuilderProps> = ({ onAddToCart }) => {
  ...
  const extraCheeseCost = cheeseLevel === 'extra' ? 25 : cheeseLevel === 'ligero' ? -5 : 0;
  ...
  return (
    <section id="builder" className="py-16 bg-[#121110] text-stone-100 border-t border-stone-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Title */}
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-950/80 border border-orange-800/60 text-amber-300 text-xs font-bold tracking-wider uppercase">
            <Sparkles className="w-3.5 h-3.5 text-orange-400" /> Creador Interactivo 2D
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-8 bg-[#181614] p-6 sm:p-8 rounded-3xl border border-stone-800/80 shadow-2xl">
            
            {/* Step 1: Tamaño */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-extrabold text-orange-400 uppercase tracking-wider">Paso 1</span>
                <span className="text-xs text-stone-400">{sizeOption.serves}</span>
              </div>
              <h3 className="text-lg font-bold text-white font-serif">Elige el Tamaño</h3>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-left transition-all ${
                      selectedSize === size.id
                        ? 'bg-gradient-to-br from-red-950 to-orange-950 border-orange-500 text-white shadow-lg'
                        : 'bg-stone-900 border-stone-800 text-stone-400 hover:border-stone-700'
                    }`}
                  >
                    <span className="font-bold text-sm block text-white">{size.name}</span>
                    <span className="text-[11px] text-stone-400 block">{size.cm}cm • {size.slices} reb.</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-stone-800/80">
              <span className="text-xs font-extrabold text-orange-400 uppercase tracking-wider">Paso 2</span>
              <h3 className="text-lg font-bold text-white font-serif">Tipo de Masa / Orilla</h3>
                ...
                {CRUST_OPTIONS.map((crust) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-xs text-white">{crust.name}</span>
                      {crust.extraCost > 0 && (
                        <span className="text-[10px] font-bold text-amber-300">+$${crust.extraCost}</span>
                      )}
                    </div>
            ...
            <div className="space-y-4 pt-4 border-t border-stone-800/80">
              <span className="text-xs font-extrabold text-orange-400 uppercase tracking-wider">Paso 3</span>
              <h3 className="text-lg font-bold text-white font-serif">Salsa de Base y Queso</h3>
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl border text-xs font-bold transition-all flex items-center gap-2 ${
                      selectedSauce === sauce.id
                        ? 'bg-stone-800 border-amber-400 text-amber-200 shadow-md'
                        : 'bg-stone-900 border-stone-800 text-stone-400 hover:border-stone-700'
                    }`}
                  >
                    <span
              ...
              <div className="flex items-center gap-3 pt-2">
                <span className="text-xs font-semibold text-stone-300">Nivel de Queso:</span>
            ...
            <div className="space-y-4 pt-4 border-t border-stone-800/80">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <span className="text-xs font-extrabold text-orange-400 uppercase tracking-wider">Paso 4</span>
                  <h3 className="text-lg font-bold text-white font-serif">Agrega tus Toppings</h3>
                </div>

                {/* Coverage mode selector */}
                <div className="flex items-center gap-1 bg-stone-950 p-1 rounded-xl border border-stone-800">
                  <span className="text-[10px] text-stone-400 px-2 font-medium">Aplicar a:</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-56 overflow-y-auto p-1 custom-scrollbar">
                {TOPPINGS.map((topping) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-stone-200">{topping.name}</span>
                        {isSelected && <Check className="w-3.5 h-3.5 text-orange-400" />}
                      </div>

                      <div className="flex items-center justify-between mt-1.5 text-[10px]">
                        <span className="text-stone-400">+${topping.price}</span>
                        {isSelected && (
                          <div
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="bg-orange-900/80 hover:bg-orange-800 px-1.5 py-0.5 rounded text-amber-200 font-bold"
                          >
            ...
            <div className="pt-2 flex justify-end">
              <button
                onClick={resetPizza}
                className="flex items-center gap-1.5 text-xs text-stone-400 hover:text-amber-400 font-semibold"
              >
          ...
          <div className="lg:col-span-5 sticky top-28 bg-[#181614] p-6 rounded-3xl border border-stone-800 shadow-2xl flex flex-col items-center justify-between space-y-6">
            
            <div className="w-full text-center">
              <span className="text-xs uppercase font-extrabold text-amber-400 tracking-wider">Vista Previa en Tiempo Real</span>
            ...
            <div className="relative w-72 h-72 sm:w-80 sm:h-80 flex items-center justify-center filter drop-shadow-[0_20px_30px_rgba(224,56,18,0.25)]">
            ...
            {/* Price & Summary Box */}
            <div className="w-full bg-stone-900/90 p-4 rounded-2xl border border-stone-800 space-y-3">
              <div className="flex items-center justify-between text-xs text-stone-300">
                <span>Ingredientes seleccionados:</span>
                <span className="font-bold text-amber-300">{Object.keys(activeToppings).length} toppings</span>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs text-stone-400 block">Total de tu creación</span>

...
import { PromosAndCombos } from './components/PromosAndCombos';
import { DeliveryChecker } from './components/DeliveryChecker';
import { ReviewsAndContact } from './components/ReviewsAndContact';
import { AdminModal } from './components/AdminModal';
import { Footer } from './components/Footer';

import { MenuItem, CartItem, Order, OrderStatus, PromoCode, Review } from './types';
import { MENU_ITEMS, PROMO_CODES, DELIVERY_ZONES, REVIEWS } from './data/menu';
...
export default function App() {
  ...
  const [storeInfo, setStoreInfo] = useState(() => {
    ...
    return {
      name: 'Pizzas Pipo Matriz',
      address: 'Av. de los Hornos #450, Col. Centro Histórico, Ciudad de México',
      phone: '555-747-6749',
      schedule: 'Lunes a Domingo: 12:00 PM – 11:00 PM',
      tickerMessage: '¡MARTES Y JUEVES DE ORILLA DE QUESO GRATIS! Usa el código PIPO20 para 20% OFF',
      ovenStatus: 'Horno de Piedra encendido a 450°C',
    };
  });
  ...
  const handleSimulateNextStatus = () => {
    ...
    const orderStatuses: OrderStatus[] = ['recibido', 'preparando', 'horneando', 'en_camino', 'entregado'];
  ...
  return (
    <div className="min-h-screen bg-[#0F0E0D] text-stone-100 font-sans antialiased selection:bg-orange-600 selection:text-white">
      ...
      <VisualPizzaBuilder onAddToCart={handleAddToCart} />

      {/* Pipo AI Sommelier */}
      <PipoAiSommelier onAddToCart={handleAddToCart} />

      {/* Promos and Combos */}
      <PromosAndCombos items={menuItems} onAddToCart={handleAddToCart} />

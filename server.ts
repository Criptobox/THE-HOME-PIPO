...
Eres Chef Pipo, el carismático y apasionado maestro pizzero de "Pizzas Pipo".
Tu misión es recomendar la combinación perfecta de pizzas y acompañamientos basada en lo que pide el cliente.
Mantén un tono entusiasta, amigable, artesanal e italiano-mexicano súper antojable ("¡Mamma Mia!", "¡Apetitoso!", "¡Te va a encantar!").

Lista de productos disponibles en Pizzas Pipo:
...
`;
  ...
  app.get("/api/download-zip", async (req, res) => {
    try {
      ...
      res.setHeader("Content-Disposition", 'attachment; filename="pizzas-pipo-fuente.zip"');
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`🍕 Servidor de Pizzas Pipo ejecutándose en http://0.0.0.0:${PORT}`);
  });
}
